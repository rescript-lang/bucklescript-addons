...
external from_options : 
  input:Bs_stream.readable ->
  output:Bs_stream.writable ->
  terminal:Js.boolean -> unit -> readline_options = "" [@@bs.obj]

external createInterface : readline_options -> t = 
...
since it's only one option, the inferred type would be

{[
  external on_line : callback -> 
]}
